# EVLaS

Enhanced Volumetric Lighting and Shadows.

- "hooks into various parts of the games weather and shadow system"
- "takes over computations of the shadow incident vector"
- "adjusts volumetric and direct lighting intensity to allow a smooth transition between day and night mode"
- "adapts itself to different weather setups by reading the current moon phase and timings at runtime, ensuring constant perfect synchronization without having to set anything up manually"

Dependencies ...

- uses SKSE
- does _not_ use KiLoader
- is _not_ dependent on ENB
- _can_ be used with Community Shaders

## Reference

- [EVLaS](https://kitsuune.pages.dev/projects/evlas/) @ Cloudflare Pages.
- [EVLaS](https://www.nexusmods.com/skyrimspecialedition/mods/63725) @ nexusmods.
